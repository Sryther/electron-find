...
class Find extends EventEmitter {
  ...
  startFind (text = '', forward = true, matchCase = false, findNext = false) {
    if (!text) return;
    this[activeMatch] = 0;
    this[matches] = 0;
    this[preText] = text;
...
function onFoundInPage (result) {
  ...
  if (typeof result.activeMatchOrdinal === 'number') {
    if (this[matches] > 1 && this[activeMatch] === result.activeMatchOrdinal ) {
      if (result.matches === result.activeMatchOrdinal) {
        this[activeMatch] = 1;
      } else {
        this[activeMatch] = result.activeMatchOrdinal + 1;
      }
    } else {
